...
export const getProgressOverview = async (userId: number) => {
  try {
    ...
    console.error('Database error in getProgressOverview:', error);
    // Return fallback data when database is unreachable
    return {
      overallPercent: 0,
      segments: [
        { label: "Lessons", percent: 0, color: "#4F46E5" },
        { label: "Quizzes", percent: 0, color: "#10B981" },
        { label: "Assignments", percent: 0, color: "#F59E0B" }
      ]
    };
  }
};
...
export const getCurrentCourses = async (userId: number) => {
  try {
    ...
    console.error('Database error in getCurrentCourses:', error);
    // Return fallback mock courses when database is unreachable
    return [
      { id: 1, slug: 'mathematics', title: 'Mathematics', subject: 'Mathematics', instructor: 'TBD', progress: 0, nextLesson: 'Introduction' },
      { id: 2, slug: 'science', title: 'Science', subject: 'Science', instructor: 'TBD', progress: 0, nextLesson: 'Introduction' },
      { id: 3, slug: 'english', title: 'English', subject: 'English', instructor: 'TBD', progress: 0, nextLesson: 'Introduction' }
    ];
  }
};
...
export const getUpcomingTasks = async (userId: number) => {
  try {
    ...
    console.error('Database error in getUpcomingTasks:', error);
...
export const getProgressSummary = async (userId: number) => {
  try {
    ...
    console.error('Database error in getProgressSummary:', error);
    // Return fallback data when database is unreachable
    return {
      overallPercent: 0,
      completedLessons: 0,
...
export const linkParent = async (userId: number, parentEmail: string, parentName: string) => {
  try {
    ...
    console.error('Database error in linkParent:', error);
    return null;
  }
};
